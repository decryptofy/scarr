...
class LRA(Engine):
    ...
    def update(self, traces: np.ndarray, plaintext: np.ndarray, average_traces, n_traces, sumsq, sums):
        ...
        sums += np.sum(traces, axis=0)
        sumsq +=  np.sum(traces**2, axis=0)
        n_traces += traces.shape[0]

        return average_traces, n_traces, sumsq, sums

    def calculate(self, average_traces, n_traces, sumsq, sums, model):
        ...
        SST = sumsq - (sums ** 2) / num_traces
        SSR = np.empty((model_vals, trace_length))

        R2s = np.empty((model_vals, trace_length))
    
        M0 = model[0].astype(np.uint32)
        ...
        WL = _WHT(traces)
        U0_iwht = _iWHT(WL.T[..., None] * U0_wht)
            
        betas = U0_iwht.transpose(1, 2, 0)
        SSR = np.sum(betas**2, axis=1)
        R2s = SSR / SST
    ...
    def find_candidate(self, R2s):
        ...
        winning_candidate = int(np.argmax(r2_peaks))
    ...
    def run_workload(self, container, tile_x, tile_y, model_pos):
        ...
        sumsq = np.zeros(container.sample_length)
        sums = np.zeros(container.sample_length)
        n_traces = 0
        ...
        for batch in container.get_batches(tile_x,tile_y):
            if traces_processed >= self.convergence_step:
                instant_r2s, _ = self.calculate(average_traces, n_traces, sumsq, sums, hypotheses)
            ...
            plaintext = batch[0]
            traces = batch[-1]
            average_traces, n_traces, sumsq, sums = self.update(traces, plaintext, average_traces, n_traces, sumsq, sums)
            traces_processed += traces.shape[0]

        instant_r2s, betas = self.calculate(average_traces, n_traces, sumsq, sums, hypotheses)
        r2s[converge_index, :, :] = instant_r2s
